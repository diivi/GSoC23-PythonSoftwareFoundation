...
4. From an open [issue](https://projects.torsion.org/borgmatic-collective/borgmatic/issues/309) in borgmatic:
   ...
   This is relevant even for the bootstrap action that I added to borgmatic, since it is also built upon the same restore functionality. If you try to run bootstrap without creating database users, it will fail, and you'll have to manually create the users and then run bootstrap again.A solution is provided in the ticket too:
...
5. During my GSoC, I worked on a [feature](https://github.com/borgmatic-collective/borgmatic/pull/60) that allows users to add constants to the top of their configuration file, and then use them in the rest of the file. This is useful for users who want to use the same value in multiple places in their configuration file, and don't want to repeat it every time. However, replacing constants does not work if you include separate configuration files in your main configuration file, and I'd like to work on a solution for this. For more information about the issue, check out the ticket [here](https://projects.torsion.org/borgmatic-collective/borgmatic/issues/745).
...
* **Writing tests** - I learned how to write tests for my code, and how to use Docker to test my code in different environments. I also learned how to mock functions and inbuilt methods in tests, how to write tests for GUIs, how to use the pytest framework, and also using the flexmock package to test my code. Some other takeaways of mine were using the pytest-qt plugin to write tests for Qt GUIs and using code coverage tools like pytest-cov to check the coverage of my tests. I had never written tests so extensively before, and I am sure this will help me in the future.
